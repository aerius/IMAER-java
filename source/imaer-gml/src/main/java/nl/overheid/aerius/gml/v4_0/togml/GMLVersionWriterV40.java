...
import nl.overheid.aerius.gml.v4_0.definitions.CustomDiurnalVariation;
 ...
 */
public class GMLVersionWriterV40 implements GMLVersionWriter {
  ...
  public GMLVersionWriterV40(final HexagonZoomLevel zoomLevel1, final String srsName) {
  ...
  private CustomDiurnalVariation convert(final nl.overheid.aerius.shared.domain.v2.characteristics.CustomDiurnalVariation customVariation) {
    ...
    gmlCustomVariation.setIntValues(customVariation.getValues().stream().map(x -> (int) Math.round(x)).collect(Collectors.toList()));
