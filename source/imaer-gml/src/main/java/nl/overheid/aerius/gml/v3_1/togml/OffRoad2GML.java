...
import nl.overheid.aerius.gml.v3_1.source.mobile.AbstractOffRoadVehicleSpecification;
import nl.overheid.aerius.gml.v3_1.source.mobile.CustomOffRoadMobileSource;
import nl.overheid.aerius.gml.v3_1.source.mobile.OffRoadMobileSource;
import nl.overheid.aerius.gml.v3_1.source.mobile.OffRoadMobileSourceProperty;
...
class OffRoad2GML extends SpecificSource2GML<OffRoadMobileEmissionSource> {
  ...
  @Override
  public nl.overheid.aerius.gml.v3_1.source.EmissionSource convert(final OffRoadMobileEmissionSource emissionSource) {
    final nl.overheid.aerius.gml.v3_1.source.mobile.OffRoadMobileEmissionSource returnSource = new nl.overheid.aerius.gml.v3_1.source.mobile.OffRoadMobileEmissionSource();
  ...
  private AbstractOffRoadVehicleSpecification toConsuming(final ConsumptionOffRoadVehicleSpecification specification) {
    final nl.overheid.aerius.gml.v3_1.source.mobile.ConsumptionOffRoadVehicleSpecification gmlSpecification = new nl.overheid.aerius.gml.v3_1.source.mobile.ConsumptionOffRoadVehicleSpecification();
  ...
  private AbstractOffRoadVehicleSpecification toOperatingHours(final OperatingHoursOffRoadVehicleSpecification specification) {
    final nl.overheid.aerius.gml.v3_1.source.mobile.OperatingHoursOffRoadVehicleSpecification gmlSpecification = new nl.overheid.aerius.gml.v3_1.source.mobile.OperatingHoursOffRoadVehicleSpecification();
    gmlSpecification.setEmissionFactors(toGmlEmissionFactors(specification.getEmissionFactors()));
  ...
  private OffRoadMobileSourceProperty toStandard(
    ...
    gmlMobileSource.setDescription(vehicleEmissionValues.getDescription());
    // With latest changes there no longer are idle properties. So can't export those any longer.
//    if (vehicleEmissionValues.getHoursIdlePerYear() != null && vehicleEmissionValues.getEngineDisplacement() != null) {
//      gmlMobileSource.setHoursIdlePerYear(vehicleEmissionValues.getHoursIdlePerYear());
//      gmlMobileSource.setEngineDisplacement(vehicleEmissionValues.getEngineDisplacement());
//    }
    return new OffRoadMobileSourceProperty(gmlMobileSource);
  }
